...
+(NSString *)dateFormateForDate:(NSDate *)date hasShort:(BOOL)hasShort
{
    ...
    return [NSString stringWithFormat:@"%04ld年%02ld月%02ld日",component.year,(long)component.month,(long)component.day];
}
...
+(NSString *)dateDescForDate:(NSDate *)date
{
    ...
    if (interval < 5*60) {
        ...
        des = [NSString stringWithFormat:@"%ld小时前",(long)(interval/3600)];
    } else if (interval < 60 * 60 * 24 * 8){
        des = [NSString stringWithFormat:@"%ld天前", (long)(interval/(3600 * 24))];
    } else {
        if (![date isThisYear]) {
