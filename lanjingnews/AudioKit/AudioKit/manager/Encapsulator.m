 ...
 *  删除数据中的第一个元素
 */
- (void)removeFirstPcmData
{
    @synchronized(pcmDatas) {
        if ([pcmDatas count] > 0) {
            [pcmDatas removeObjectAtIndex:0];
        }
    }
}
...
- (void)prepareForEncapsulating {
    ...
    self.isCanceled = NO;
    if (encapsulationOperation) {        
        [encapsulationOperation cancel];
        encapsulationOperation = nil;
    }
    encapsulationOperation = [[EncapsulatingOperation alloc] initWithParent:self];
    if (encapsulationOperation == nil) {
        return;
    }
    if (operationQueue) {
...
//            }
            [self.mParent removeFirstPcmData];
    ...
    if ([self isCancelled]) {
        //canceled
    }else if ( ! [self.mParent isCanceled]) {
