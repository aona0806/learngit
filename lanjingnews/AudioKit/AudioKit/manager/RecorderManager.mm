...
#define kUseSession  1
#define kMaxDuration   60
...
#if  kUseSession
        [[NSNotificationCenter defaultCenter]addObserver:self selector:@selector(sessionInterruptNotification:) name:AVAudioSessionInterruptionNotification object:nil];
        [[NSNotificationCenter defaultCenter]addObserver:self selector:@selector(sessionRouteChangeNotification:) name:AVAudioSessionRouteChangeNotification object:nil];
#endif
    }
    return self;
}
...
- (void)startRecording {
    ...
    if ( ! mAQRecorder) {
        ...
        if (!success) {
            NSLog(@"av audio session set category: %@ failed for: %@",AVAudioSessionCategoryPlayAndRecord,error);
            return;
        }
        ...
        if (error) {
            ...
            NSLog(@"av audio session set active failed for: %@",error);
            return;
        }
#else
        //*
    ...
    if (mAQRecorder &&  ! mAQRecorder->IsRunning()) {
        startRecord = mAQRecorder->StartRecord(self.encapsulator);
    }
    if (!startRecord || ! mAQRecorder->IsRunning()) {
        mAQRecorder->StopRecord();
        //从operation queue里面去掉
        [self.encapsulator resetWithFileName:filename];
    ...
    self.timerTimeout = [NSTimer scheduledTimerWithTimeInterval:kMaxDuration target:self selector:@selector(timeoutCheck:) userInfo:nil repeats:NO];
}
...
#if  kUseSession
-(void)sessionInterruptNotification:(NSNotification *)notification
{
...
-(void)sessionRouteChangeNotification:(NSNotification *)notification
{
    ...
    NSNumber *numReason = userInfo[AVAudioSessionRouteChangeReasonKey];
    NSUInteger reason = [numReason integerValue];
