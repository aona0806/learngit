...
-(void)Captureimage
{
    ...
    [self.captureOutput captureStillImageAsynchronouslyFromConnection:videoConnection completionHandler:
     ^(CMSampleBufferRef imageSampleBuffer, NSError *error) {
         ...
         t_image = [self fixImageSize:t_image];
...
-(void)takePhoto
{
    if ([UIImagePickerController isSourceTypeAvailable: UIImagePickerControllerSourceTypeCamera])
    {
        ...
        camreaVc.maxCount = self.maxCount - self.images.count;
...
-(void)LocalPhoto
{
    ...
    if (author == ALAuthorizationStatusRestricted || author == ALAuthorizationStatusDenied){
        //无权限
        UIAlertView *alertView = [[UIAlertView alloc] initWithTitle:@"温馨提示" message:@"请为蓝鲸财经开放相册权限，手机设置－隐私－相片－蓝鲸财经（打开）" delegate:nil cancelButtonTitle:@"我知道了" otherButtonTitles:nil, nil];
        [alertView show];
        return;
    }
...
- (UIImage *)fixImageSize:(UIImage *)srcImage
{
    UIImage *reSizeImage = srcImage;
    //    NSString *deviceString = [[UIDevice currentDevice]platformString];
    if (SCREEN_HEIGHT <= 640) { // iphone 4
        
        CGFloat width = srcImage.size.width;
        CGFloat height = srcImage.size.height;
        CGFloat reHeight = SCREEN_HEIGHT * width / SCREEN_WIDTH;
        
        CGRect rect = CGRectMake(0, (height - reHeight) / 2, width, reHeight);
