...
- (id)initWithFrame:(CGRect)frame items:(NSArray *)items;
{
    ...
    if (self) {
        ...
        float init_x          =0;//= CGRectGetMinX(frame);
        float init_y          =0;//= CGRectGetMinY(frame);
        ...
        if (items.count >= 2) {
            for (NSInteger i =0; i<items.count; i++) {
                RFSegmentItem *item = [[RFSegmentItem alloc] initWithFrame:CGRectMake(init_x, init_y, itemWidth, itemHeight)
                                                                  ...
                                                                  norColor:RGBA_Color(241, 241, 241, 1)
...
- (void)updateItem:(NSString *)item atIndex:(NSInteger)index
{
    if (index < 0 || index > self.itemsArray.count) {
        return;
    }
    RFSegmentItem *segItem = (RFSegmentItem *)[self.itemsArray objectAtIndex:index];
    segItem.titleLabel.text = item;
}
