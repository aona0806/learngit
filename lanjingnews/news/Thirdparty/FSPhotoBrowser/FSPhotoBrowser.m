...
//                    zoomView.imageView.frame = _currentImageViewFrame;

                    zoomView.imageView.alpha = 1.0;
                    
                    zoomView.imageView.contentMode = UIViewContentModeScaleAspectFill;
...
- (void)resetZoomImageViewsFrameAtIndex:(NSInteger)index
{
    for (NSInteger i = index; i < FS_PHOTO_BROWSER_SCROLL_IMAGE_VIEW_MAX_ZOOM_IMAGES + index; i++) {
        ...
        if (_dataSource && [_dataSource respondsToSelector:@selector(photoBrowser:requestDataWithType:atIndex:)]) {
            NSObject *object = [_dataSource photoBrowser:self requestDataWithType:PhotoBrowerScrollRequestDataTypeForNull atIndex:i];
            if ([object isKindOfClass:[NSString class]]) {
                NSString *url = (NSString *)object;
                
                UIImage *placeHolderImage = nil;
                if ([_dataSource respondsToSelector:@selector(imagePhotoPlaceHolder:atIndex:)]) {
                    placeHolderImage = [_dataSource imagePhotoPlaceHolder:self atIndex:i];
                }
                
                if (url) {
                    
                    if (placeHolderImage) {
                        [zoomImage setImageWithUrl:url andPlaceHolderImage:placeHolderImage];
                    }else {
...
- (void)resetZoomImageViewWithLocation:(NSInteger)location orientation:(PhotoBrowerScrollOrientation)orientation
{
    ...
    if (_dataSource && [_dataSource respondsToSelector:@selector(photoBrowser:requestDataWithType:atIndex:)]) {
        NSObject *object = [_dataSource photoBrowser:self requestDataWithType:PhotoBrowerScrollRequestDataTypeForNull atIndex:index];
        if ([object isKindOfClass:[NSString class]]) {
            NSString *url = (NSString *)object;
            UIImage *placeHolderImage = nil;
            if ([_dataSource respondsToSelector:@selector(imagePhotoPlaceHolder:atIndex:)]) {
                placeHolderImage = [_dataSource imagePhotoPlaceHolder:self atIndex:index];
            }
            
            if (url) {
                
                if (placeHolderImage) {
                    [zoomImageView setImageWithUrl:url andPlaceHolderImage:placeHolderImage];
                }else {
