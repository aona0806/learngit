...
- (NSArray *)daysOfWeek
{
    ...
    switch(self.calendarManager.calendarAppearance.weekDayFormat){
        ...
        case JTCalendarWeekDayFormatShort:
            for(NSInteger i = 0; i < days.count; ++i){
                NSString *day = days[i];
                if ([[[day uppercaseString] substringToIndex:1] isEqualToString: @"星"])
                {
                    [days replaceObjectAtIndex:i withObject:[[day uppercaseString] substringWithRange:NSMakeRange(2, 1)]];
                }else{
