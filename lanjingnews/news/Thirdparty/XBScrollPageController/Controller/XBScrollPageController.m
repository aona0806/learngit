...
@property (nonatomic,strong) NSArray<UIViewController *> *displayClasses; /**< 要展示的子控制器类名,如果只传一个则表示重复使用该控制器类  */
...
- (void)viewDidLayoutSubviews
{
    ...
    self.tagCollectionView.frame = CGRectMake(0, _titleVerticalOffset, XBScreenWidth, self.tagViewHeight);
    
    CGRect frame = CGRectMake(0, CGRectGetMaxY(self.tagCollectionView.frame), XBScreenWidth, self.view.frame.size.height - CGRectGetMaxY(self.tagCollectionView.frame));
    if (!CGRectEqualToRect(frame, self.pageCollectionView.frame)) {
...
- (void)dealloc
{
    ...
    self.pageCollectionView.delegate = nil;
    self.tagCollectionView.delegate = nil;
}
...
- (void)setupCollectionView
{
    ...
    pageFlowLayout.sectionInset = UIEdgeInsetsZero;
...
//        cell.backgroundColor = XBRandomColor;
        ...
        //更新page状态
        UIViewController *displayClass = (self.displayClasses.count == 1)?[self.displayClasses firstObject]:self.displayClasses[indexPath.item];
        ...
        if (!cachedViewController) {   //如果缓存里不存在,生成新的VC,并加入缓存(如果缓存里存在,表明之前alloc过,直接使用 )
            cachedViewController = displayClass;
        }
        //更新缓存
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if ([self isTagView:collectionView]) {                     //选中某个标签
        ...
        if (self.selectedIndex == indexPath.item && self.isScroll == NO) {
            [self didSelectedAtIndex:indexPath.item];
        }
