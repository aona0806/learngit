...
#define DDCOUNT_DOWN_TIME                               10

@interface RecordingView()

@property(nonatomic , strong) NSTimer *countdownTimer;
@property(nonatomic , assign) NSInteger timeLaspe;//自从显示开始的时间
...
{
    if (self.timeLeftLabel.superview) {
        //需要重新设置显示
...
- (void)setupCancelSendView
{
    ...
    UIImage* image = [UIImage imageNamed:@"meet_cancel_send_record"];
...
- (void)setupShowVolumnState
{
    ...
    UIImage* image = [UIImage imageNamed:@"meet_recording"];
    UIImageView* imageView = [[UIImageView alloc] initWithImage:image];
    ...
    UIImage* volumnImage = [UIImage imageNamed:@"meet_volumn"];
...
- (void)setupShowRecordingTooShort
{
    ...
    UIImage* image = [UIImage imageNamed:@"meet_record_too_short"];
    UIImageView* imageView = [[UIImageView alloc] initWithImage:image];
...
-(void)showCountdown
{
    if (_timeLeftLabel == nil) {
        ...
        _timeLeftLabel.font = [UIFont systemFontOfSize:80];
    }
    if (_timeLeftLabel.superview == nil) {
        [self.subviews enumerateObjectsUsingBlock:^(id obj, NSUInteger idx, BOOL *stop) {
            if ([(UIView*)obj tag] != 100)
            {
                [(UIView*)obj removeFromSuperview];
    ...
    _timeLeftLabel.text = [NSString stringWithFormat:@"%ld",(long)(self.maxShowTime - _timeLaspe)];
}
...
{
    ++_timeLaspe;
    if (_timeLaspe > self.maxShowTime) {
        [self.countdownTimer invalidate];
        return;
    }
    if (self.maxShowTime - _timeLaspe < DDCOUNT_DOWN_TIME) {
        [self showCountdown];
    }
}
