...
@property (nonatomic, strong) LJUserNoteDataDataModel *model;
...
- (void) updateInfo:(LJUserNoteDataDataModel *)info
{
    self.model = info;
    
    NSString *userId = [AccountManager.sharedInstace uid];
    ...
    [self.headImageView sd_setImageWithURL:[NSURL URLWithString:avatarString] placeholderImage:[UIImage imageNamed:@"new_defaultHeader"]];

    //!!!: 默认数据改为"--"
    NSString *friendsNumString = info.friendsNum == nil ? @"--" : info.friendsNum;
...
-(void)focusMeButtonClick:(UIButton *)button
{
    ...
    [[TKRequestHandler sharedInstance] postFocusSomeOneWithFollower:!isFollow FollowerUid:self.model.uid complated:^(NSURLSessionDataTask *sessionDataTask, LJRelationFollowModel *model, NSError *error) {
        ...
        if (!error) {
            if (model.dErrno.integerValue == 0) {
                ...
                if (relationFollowDataModel) {
                    myself.model.userRelation.followType = relationFollowDataModel.type;
                }
                
                if (myself.ljDelegate && [myself.ljDelegate respondsToSelector:@selector(changeTheFocunRelationWithType:userDetailHeaderTableViewCell:)]) {
                    [myself.ljDelegate changeTheFocunRelationWithType:myself.model.userRelation.followType userDetailHeaderTableViewCell:myself];
                }
            }
        }
    }];
