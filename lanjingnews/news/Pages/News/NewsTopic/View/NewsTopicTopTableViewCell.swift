//
//  NewsTopicTop.swift
//  news
//
//  Created by 陈龙 on 16/1/8.
//  Copyright © 2016年 lanjing. All rights reserved.
//

import UIKit

class NewsTopicTopTableViewCell: BaseTableViewCell {

    class Consts: NSObject {
        
        static let TitleBackImageRatio: CGFloat = 40.0 / 375
        static let TitleColor = UIColor.rgb(0xffffff)
        static let TitleFont = UIFont.systemFont(ofSize: 20)
        static let TitleHorizontalSpace: CGFloat = 15.0
        static let TitleVerticalSpace: CGFloat = 10.0
        
        static let ContentHorizontalSpace: CGFloat = 15.0
        static let ContentVerticalSpace: CGFloat = 20.0
        static let LineHeight: CGFloat = 0.5
    }
    
    private let postImageView: UIImageView! = UIImageView()
    private let titleBackImageView: UIImageView! = UIImageView()
    private let titleLabel: UILabel! = UILabel()
    private let contentLabel = UILabel()
    private let lineSpace = UIView()

    // MARK: - Lifecycle
    
    override init(style: UITableViewCellStyle, reuseIdentifier: String?) {
        
        super.init(style: style, reuseIdentifier: reuseIdentifier)
        
        self.selectionStyle = .none
        buildView()
        
    }
    
    required init?(coder aDecoder: NSCoder) {
        fatalError("init(coder:) has not been implemented")
    }
    
    // MARK: - public 
    
    var info: NewsTopicDetailDataTopicInfoModel? {
        didSet {
            
            let postImageUrl = Urlhelper.tryEncode(info?.focusImg)
            let defaultImage = UIImage(named: "default_news_activity")
            postImageView.sd_setImage(with: postImageUrl, placeholderImage: defaultImage)
            let contentString = "摘要：" + (info?.desc ?? "")
            contentLabel.text = contentString
            
            postImageView.snp.updateConstraints { (make) -> Void in
                make.left.top.right.equalTo(contentView)
                let height: CGFloat = NewsConfig.PostImageRatio * GlobalConsts.screenWidth
                make.height.equalTo(height)
            }
            
            let title = info?.title ?? ""
            let titleLabelWidth = GlobalConsts.screenWidth - Consts.TitleHorizontalSpace * 2
            let titleLabelHeight = title.size(withMaxWidth: titleLabelWidth, font: Consts.TitleFont, maxLineNum: 2).height 
            
            titleBackImageView.snp.updateConstraints { (make) -> Void in
                make.left.right.equalTo(postImageView)
                let titleBackImageViewHeight = titleLabelHeight > 0 ? titleLabelHeight + Consts.TitleVerticalSpace * 2 : 0
                make.height.equalTo(titleBackImageViewHeight)
                make.bottom.equalTo(postImageView.snp.bottom)
            }
            
            titleLabel.text = title
            titleLabel.snp.updateConstraints { (make) -> Void in
                make.height.equalTo(titleLabelHeight)
                make.left.equalTo(titleBackImageView.snp.left).offset(Consts.TitleHorizontalSpace)
                make.right.equalTo(titleBackImageView.snp.right).offset(-Consts.TitleHorizontalSpace)
                make.bottom.equalTo(titleBackImageView.snp.bottom).offset(-Consts.TitleVerticalSpace)
                make.top.equalTo(titleBackImageView.snp.top).offset(Consts.TitleVerticalSpace)
            }
            
            contentLabel.snp.updateConstraints { (make) -> Void in
                make.left.equalTo(contentView.snp.left).offset(Consts.ContentHorizontalSpace)
                make.right.equalTo(contentView.snp.right).offset(-Consts.ContentHorizontalSpace)
                make.top.equalTo(postImageView.snp.bottom).offset(Consts.ContentVerticalSpace)
                let contetnWidth: CGFloat = GlobalConsts.screenWidth - Consts.ContentHorizontalSpace * 2
                let contentHegith = contentString.size(withMaxWidth: contetnWidth, font: NewsConfig.DetailContentFont).height 
                make.height.equalTo(contentHegith)
            }
            
            lineSpace.snp.updateConstraints { (make) -> Void in
                make.left.equalTo(contentLabel.left).offset(Consts.ContentHorizontalSpace)
                make.right.equalTo(contentLabel.right).offset(-Consts.ContentHorizontalSpace)
                make.bottom.equalTo(contentView.snp.bottom).offset(-Consts.LineHeight)
                make.height.equalTo(Consts.LineHeight)
            }
        }
    }
    
   class func heightForCell(_ info: NewsTopicDetailDataTopicInfoModel?) -> CGFloat {
        
        var height: CGFloat = 0
        if info != nil {
            let imageHeight: CGFloat = NewsConfig.PostImageRatio * GlobalConsts.screenWidth
            let contentWidth: CGFloat = GlobalConsts.screenWidth - Consts.ContentHorizontalSpace * 2
            let descString = "摘要：" + (info?.desc ?? "" )
            let contentHegith = descString.size(withMaxWidth: contentWidth, font: NewsConfig.DetailContentFont).height 
            height = imageHeight + contentHegith + Consts.ContentVerticalSpace * 2
        }
        return height
    }
    
    // MARK: - private
    
    private func buildView() {
        
        contentView.addSubview(postImageView);
        
        self.titleBackImageView.image = UIImage(named: "news_banner_transparentgray")
        postImageView.addSubview(titleBackImageView)

        titleLabel.textColor = Consts.TitleColor
        titleLabel.font = Consts.TitleFont
        titleLabel.numberOfLines = 2
        titleLabel.lineBreakMode = .byTruncatingTail
        titleBackImageView.addSubview(titleLabel)
        
        contentLabel.textColor = NewsConfig.DetailContentColor
        contentLabel.font = NewsConfig.DetailContentFont
        contentLabel.numberOfLines = 0
        contentView.addSubview(contentLabel)
        
        lineSpace.backgroundColor = NewsConfig.SeperateLineColor
        contentView.addSubview(lineSpace)
    }
    
}
