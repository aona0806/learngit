...
class FavoriteOneImageCell: UITableViewCell {
    ...
    override func updateConstraints() {
        ...
        titleLabel.snp.updateConstraints { (make) -> Void in
            ...
            make.right.equalTo(postImageView.snp.left).offset(-24)
        ...
        authorInfoLabel.snp.updateConstraints { (make) -> Void in
            ...
            make.right.equalTo(self.postImageView.snp.right).offset(-topHeight)
        }
    ...
    class func heightForInfo(_ model: LJFavoriteDataModel) -> CGFloat {
        ...
        let titleSize: CGSize! = titleString.size(withMaxWidth: UIScreen.main.bounds.width - 30 - 137, font: NewsConfig.TitleFont, maxLineNum: 2)
        var height = titleSize.height + 85
    ...
    func setValueWithModel(_ model:LJFavoriteDataModel) {
        ...
        let favType = model.info?.favType ?? "3"
        if favType == "3" {
        ...
        let urlString: String = model.info!.imgs?[0] ?? ""
        postImageView.sd_setImage(with: Urlhelper.tryEncode(urlString), placeholderImage: UIImage(named: "default_news_news"))
        
        let removed = model.info?.isDel?.intValue ?? 0
        if removed == 1{
            deleteView.alpha = 1
